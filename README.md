...
exploration on a live graph, per-account neighborhoods, plain-English risk
explanations (optionally generated by an LLM via `OPENAI_API_KEY`, with a
deterministic local fallback), and an **investigation assistant** panel where
you can ask natural-language questions like "why is acc_7 risky?" or "describe
...
Every pull request gets a review from a coding agent: the
[`.github/workflows/pr-review.yml`](.github/workflows/pr-review.yml) workflow
runs the [Codex CLI](https://developers.openai.com/codex/) over the diff and
posts a maintainer-style comment (correctness, security, honest results, test
coverage, style) with a `Verdict: LGTM` / `Verdict: changes requested` line.
It activates only when the repo has a `CODEX_API_KEY` (or `OPENAI_API_KEY`)
secret and otherwise skips silently, so it never blocks contributions from
without-model reviews. To use it: add a `CODEX_API_KEY` secret in
